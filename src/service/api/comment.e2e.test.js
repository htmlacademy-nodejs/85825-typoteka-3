...
const createAPI = () => {
  ...
  comment(app, new ArticleService(cloneData), new CommentService());
...
describe(`API returns a list of comments to given article`, () => {
  ...
  beforeAll(async () => {
    const app = createAPI();
    response = await request(app).get(`/articles/P1h75L/comments`);
  });

  test(`Status code 200`, () => expect(response.statusCode).toBe(HttpCode.OK));
  test(`Returns list of 3 comments`, () => expect(response.body.length).toBe(4));
  test(`First comment's id is ofThKs`, () => expect(response.body[0].id).toBe(`ofThKs`));
});
...
describe(`API creates a comment if data is valid`, () => {
  ...
  beforeAll(async () => {
    app = createAPI();
    response = await request(app).post(`/articles/P1h75L/comments`).send(newComment);
  });
  ...
  test(`Comments count is changed`, async () => {
    await request(app).get(`/articles/P1h75L/comments`)
...
test(`API refuses to create a comment when data is invalid, and returns status code 400`, async () => {
  const app = createAPI();
  await request(app).post(`/articles/3Bx_Qz/comments`).send({})
...
describe(`API correctly deletes a comment`, () => {
  ...
  beforeAll(async () => {
    app = createAPI();
    response = await request(app).delete(`/articles/UmqfWT/comments/dy4Isr`);
  });

  test(`Status code 200`, () => expect(response.statusCode).toBe(HttpCode.OK));
  test(`Returns comment deleted`, () => expect(response.body.id).toBe(`dy4Isr`));
  test(`Comments count is 2 now`, async () => {
    await request(app).get(`/articles/UmqfWT/comments`)
